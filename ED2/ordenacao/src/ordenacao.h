...
int ordSelecaoSecond(int tam, reg r[]){
...
void ordHeapSecond(int n, reg r[], int *cont)
{
	int i = n/2;
	int pai, filho;
	reg t;
	...
	for(;;)
	{
		...
		while(filho < n)
		{
			if((filho + 1 < n) && (r[filho + 1].valor > r[filho].valor))
			{
				filho++;
			}
			if(r[filho].valor > t.valor){
				r[pai] = r[filho];
				pai = filho;
				filho = pai *2+1;
				*cont = *cont + 1;
...
int ordShellSecond(int tam, reg r[]) 
{
	int inte = 0;
    int i , j;
    reg valor;
    ...
    while ( gap > 1) {
        ...
        for(i = gap; i < tam; i++) {
            valor = r[i];
            j = i - gap;
            while (j >= 0 && valor.valor < r[j].valor) {
                r[j + gap] = r[j];
                j -= gap;
                inte++;
            }
...
int quickSortSecond(int inicio, int fim, int inte, reg r[])
{
   int pivo, i, j, meio;
   reg aux;
   ...
   inte = 0;
   do{
      while (r[i].valor < pivo) i = i + 1;
      while (r[j].valor > pivo) j = j - 1;
   ...
   if(inicio < j) inte += quickSortSecond(inicio, j, inte, r);
...
int ordQuick(int tam, reg r[], int opcao){
	int inte;
	if(opcao == 1) inte = quickSort(0, tam+1, 0, r);
	else if(opcao == 2) inte = quickSortSecond(0, tam+1, 0, r);
	return inte;
}
