...
/* Autores: José Diôgo, Fernando Maia, Wellington Correia, Mateus Nascimento
 * 
 * 
 **/
#include <string.h>
...
//funcoes para primeira arvore - Por nome
...
//Carrega os registros na arvore - Por telefone
void carregarArvore(Apontador *apontador)
{
	Contato c;
...
void InsereNaPagina(Apontador Ap, Contato Reg, Apontador ApDir){
  ...
  while (NaoAchouPosicao)
  {
    if (strcmp(Reg.nome,Ap->contato[k - 1].nome) == 0 || strcmp(Reg.nome,Ap->contato[k - 1].nome) > 0)//Reg.chave >= Ap->contato[k - 1].chave)
    {
      NaoAchouPosicao = 0;
      break;
    }
...
void buscainFileSmartEmail2(Contato Reg, Apontador pag)
{
    ...
    fclose(arq);
    for(i = 0; i < 2*ORDEM; i++){
        if(strcmp(reg[i].email, Reg.email) == 0 && strcmp(reg[i].telCelular,"") != 0)
        { 
		 printf("\n  Nome: %s\n\n", reg[i].nome);
         printf("  CPF: %s\n", reg[i].cpf);
         printf("  Data de nascimento: %d/%d/%d\n", reg[i].dataNascimento.dia, reg[i].dataNascimento.mes, reg[i].dataNascimento.ano);
...
void ins2(Contato contato, Apontador apontador, int *cresceu, Contato *contatoRetorno, Apontador *apontadorRetorno)
{
	...
	{
		printf(" Já existe um contato com este contato. Verifique o número do contato e tente novamente :(\n");
		return;
	}
...
void InsereNaPagina2(Apontador Ap, Contato Reg, Apontador ApDir){
  ...
  while (NaoAchouPosicao)
  {
    if (strcmp(Reg.telCelular,Ap->contato[k - 1].telCelular) == 0 || strcmp(Reg.telCelular,Ap->contato[k - 1].telCelular) > 0)//Reg.chave >= Ap->contato[k - 1].chave)
    {
      NaoAchouPosicao = 0;
      break;
    }
