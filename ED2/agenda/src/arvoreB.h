...
void buscainFile(Contato Reg, Apontador pag)
{
    Contato reg[2*ORDEM];
    int i;
    FILE *arq = fopen("_arquivo.dat","rb");
    if (arq == NULL)
    exit(1);
...
void ins(Contato contato, Apontador apontador, int *cresceu, Contato *contatoRetorno, Apontador *apontadorRetorno)
{
	...
	{
		printf("Este contato ja se encontra na lista: %s \n", contato.nome);
		return;
	}
...
void InsereNaPagina(Apontador Ap, Contato Reg, Apontador ApDir){
  ...
  while (NaoAchouPosicao)
  {
    if (strcmp(Reg.cpf,Ap->contato[k - 1].cpf) == 0 || strcmp(Reg.cpf,Ap->contato[k - 1].cpf) > 0)//Reg.chave >= Ap->contato[k - 1].chave)
    {
      NaoAchouPosicao = 0;
      break;
    }
...
void Ret(char Ch[], Apontador *Ap, int *Diminuiu){
  int Ind, j;
  Apontador WITH;
  Contato Reg;
  ...
  WITH = *Ap;
  Ind = 1;
  while (Ind < WITH->numChaves && strcmp(Ch, WITH->contato[Ind - 1].cpf) > 0)//Ch > WITH->contato[Ind - 1].chave) 
    Ind++;
  if (strcmp(Ch, WITH->contato[Ind - 1].cpf) == 0)//Ch == WITH->contato[Ind - 1].chave)
  {
    strcpy(Reg.cpf," ");
  ...
  if (strcmp(Ch,WITH->contato[Ind - 1].cpf)  > 0)//Ch > WITH->contato[Ind - 1].chave)
    Ind++;
  Ret(Ch, &WITH->apontador[Ind - 1], Diminuiu);
  if (*Diminuiu)
    Reconstitui(WITH->apontador[Ind - 1], *Ap, Ind - 1, Diminuiu);
}  /* Ret */
